...
function FeatureCard({ icon: Icon, title, description, animationDelay = '0s', shouldAnimate }: FeatureCardProps) {
  return (
    <Card
      className={cn(
...
const teamMembers: TeamMember[] = [
  { name: "Alice Dupont", role: "Lead Developer", imageUrl: "https://placehold.co/300x300.png", avatarFallback: "AD", imageHint: "portrait professional" },
  { name: "Bob Martin", role: "UI/UX Designer", imageUrl: "https://placehold.co/300x300.png", avatarFallback: "BM", imageHint: "portrait creative" },
  { name: "Charles Leclerc", role: "AI Specialist", imageUrl: "https://placehold.co/300x300.png", avatarFallback: "CL", imageHint: "portrait tech" },
  { name: "Diana Moreau", role: "Cybersecurity Analyst", imageUrl: "https://placehold.co/300x300.png", avatarFallback: "DM", imageHint: "portrait security" },
];
...
function TeamMemberCard({ member, animationDelay = '0s', shouldAnimate }: TeamMemberCardProps) {
  return (
    <Card
      className={cn(
    ...
    >
      <Avatar className="w-32 h-32 mx-auto mb-4 border-4 border-primary/30 shadow-md">
        ...
        <AvatarFallback className="text-3xl bg-primary/20 text-primary">{member.avatarFallback}</AvatarFallback>
      </Avatar>
...
const DecorativeHoop = ({ size, position, animationDuration, animationName = "animate-spin-slow" }: { size: number, position: string, animationDuration: string, animationName?: string }) => {
  return (
    <div
      className={`absolute ${animationName} ${position} pointer-events-none opacity-50 md:opacity-70 z-0`}
      style={{ animationDuration }}
    >
      <Image
        src="/iridescent-hoop.png"
        alt=""
        width={size}
        height={size}
...
export default function LandingPage() {
  const featuresData: Omit<FeatureCardProps, 'animationDelay' | 'shouldAnimate'>[] = [
    {
      icon: Mic,
  ...
  const createObserverCallback = (setter: React.Dispatch<React.SetStateAction<boolean>>) => {
  ...
  useEffect(() => {
    const heroObserver = new IntersectionObserver(createObserverCallback(setHeroContentVisible), { threshold: 0.1 });
    const featuresObserver = new IntersectionObserver(createObserverCallback(setFeaturesVisible), { threshold: 0.1 });
    const teamObserver = new IntersectionObserver(createObserverCallback(setTeamVisible), { threshold: 0.1 });
    const footerObserver = new IntersectionObserver(createObserverCallback(setFooterVisible), { threshold: 0.1 });
    ...
    return () => {
      if (heroContentRef.current) heroObserver.unobserve(heroContentRef.current);
  ...
  return (
    <div className="flex flex-col min-h-screen text-foreground overflow-x-hidden bg-background">
      ...
      <section className="relative flex flex-col lg:flex-row items-center justify-center min-h-screen p-8 lg:p-16 overflow-hidden">
        ...
        >
          ...
          <h1 className={cn(
              ...
              heroContentVisible ? "animate-emerge-up animation-delay-200" : "opacity-0"
            )}
          >
          ...
          <p className={cn(
              ...
              heroContentVisible ? "animate-emerge-up animation-delay-400" : "opacity-0"
            )}
          >
            Sécurisez votre monde numérique, une conversation à la fois.
          </p>
          <div className={cn(heroContentVisible ? "animate-emerge-up animation-delay-600" : "opacity-0")}>
            <Link href="/auth" passHref>
              <Button size="lg" className="group bg-primary hover:bg-primary/90 text-primary-foreground shadow-lg hover:shadow-primary/40 transition-all duration-300 transform hover:scale-105 px-10 py-6 text-lg">
                Démarrer
        ...
        >
          <div className="relative w-[300px] h-[400px] md:w-[400px] md:h-[500px] lg:w-[500px] lg:h-[600px] animate-float-subtle" style={{animationDuration: '8s'}}>
            <Image
            ...
            />
            <DecorativeHoop size={150} position="top-[5%] left-[0%]" animationDuration="20s" />
            <DecorativeHoop size={200} position="bottom-[10%] right-[10%]" animationDuration="25s" animationName="animate-spin-slow" />
          </div>
        </div>
      </section>

      {/* Features Section */}
      <section ref={featuresSectionRef} className="py-16 lg:py-24 bg-muted/60 z-10 relative border-t border-border/30">
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="text-center mb-12 lg:mb-16">
            ...
            <p className={cn(
                ...
                featuresVisible ? "animate-emerge-up animation-delay-200" : "opacity-0"
              )}
            >
      ...
      {/* Team Section */}
      <section ref={teamSectionRef} className="py-16 lg:py-24 bg-background z-10 relative border-t border-border/30">
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="text-center mb-12 lg:mb-16">
            <h2 className={cn(
                "text-3xl md:text-4xl font-bold font-headline text-primary mb-4",
            ...
            <p className={cn(
                "text-lg text-muted-foreground max-w-2xl mx-auto",
                teamVisible ? "animate-emerge-up animation-delay-200" : "opacity-0"
              )}
            >
      ...
      {/* Footer */}
      <footer ref={footerRef} className={cn(
